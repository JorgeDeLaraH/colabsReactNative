...
interface FormData {
  strSkillName: '.Net' | 'React' | 'React Native'  | 'Angular' | 'Ionic' | 'Python' | 'SQL' | 'Flutter'
  strLevel: 'Básico' | 'Intermedio' | 'Avanzado'
  numYearNum:string;
  
}

export const NewSkill:React.FC<{navigation:any}> = ({navigation}) => {
  const { control, handleSubmit, setValue } = useForm<FormData>();
  const [skills,setSkills]=useState<string[]>(['.Net' , 'React' , 'React Native'  , 'Angular' , 'Ionic' , 'Python' , 'SQL' , 'Flutter'])
  const [levels,setLevels]=useState<string[]>(['Básico' , 'Intermedio' , 'Avanzado'])
  ...
  const onSubmit=(data:FormData)=>{
    ...
    const datos={
      id:ide,
      skill:data.strSkillName,
      level:data.strLevel,
      numYear:year

    }
    console.log(datos)
    axios.post(`http://192.168.18.6:9005/skills`,datos)
    .then((res:any)=>{
